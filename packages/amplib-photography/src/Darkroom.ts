...
export class Darkroom {
  ...
  develop(params: DevelopParams, scale = 1): void {
    ...
    // The aperture defocus reads a wide quarter-res blur of the image,
    // skipped when the aperture is closed — the composite still samples the
    // target, but multiplied by 0.
    if (params.aperture > 0) {
      // downsample-and-blur lin at quarter res, then one widening iteration —
      // ping-pong ending in defocusA
